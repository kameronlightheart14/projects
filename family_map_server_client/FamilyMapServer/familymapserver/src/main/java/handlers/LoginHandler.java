...
/**
 * Handler object that handles Login requests
 *
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            if (exchange.getRequestMethod().toLowerCase().equals("post")) {
                ...
                regRequest = (LoginRequest) requests.JsonPacket.deserialize(reqData, LoginRequest.class);
                // Run the register request
                LoginResponse loginResponse = LoginService.login(regRequest);
                // Send OK http response
                exchange.sendResponseHeaders(HttpURLConnection.HTTP_OK, 0);
                // Send response (login response)
                OutputStream responseBody = exchange.getResponseBody();
                System.out.println(loginResponse.serialize());
                String responseJson = loginResponse.serialize();
